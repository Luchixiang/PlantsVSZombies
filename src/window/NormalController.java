...
public class NormalController implements Initializable {
    ...
    @FXML
    private AnchorPane normalMapPane;
    @FXML
    ...
    private int theRestZombie;
    private NormalCallBack normalCallBack;
    private volatile int[] isHaveZombie = {0, 0, 0, 0, 0};
    private boolean gameover;
    private boolean isPause = false;
    private boolean isShovel;
    private int testNumber = 0;
    private Timer timer = new Timer();
    AudioPlayer audioPlayer = new AudioPlayer(new File("@../../audio/Faster.mp3"));
    ...
    public void initialize(URL location, ResourceBundle resources) {
        normalCallBack = new NormalCallBack();
        addUtil = new AddUtil(normalCallBack);

        System.out.println(chooseMap.getMapNumber());
    ...
    private void setMapSun() {
        Image normalMapImage = new Image(new File("@../../images/interface/background1.jpg").toURI().toString());
        NormalMap.setImage(normalMapImage);
    }

    private void setMapNight() {
        Image normalMapImage = new Image(new File("@../../images/interface/background2.jpg").toURI().toString());
        NormalMap.setImage(normalMapImage);
    }

    @FXML
    private void showHand() {
        normalMapPane.setCursor(Cursor.HAND);
    }

    @FXML
    private void hideHand() {
        normalMapPane.setCursor(Cursor.DEFAULT);
    }
    ...
    private void mouseMoved() {
        for (Node node : normalMapPane.getChildren()) {
    ...
    private class EnterHandler implements EventHandler<Event> {
        ...
        public void handle(Event event) {
            ...
            if (cardSelected && source.getImage() == null && wantedPlantImage != null) {
                ...
                if (wantedPlant instanceof EatFlower || wantedPlant instanceof TallNut) {
    ...
    private class ClickHandler implements EventHandler<MouseEvent> {
        ...
        public void handle(MouseEvent event) {
            if (!gameover && !isPause) {
                if (cardSelected && wantedPlant != null && !(wantedPlant instanceof LilyPad)) {
...
//                    source.setFitHeight(wantedPlantImage.getHeight());
//                    source.setFitWidth(wantedPlantImage.getWidth());
                            if (wantedPlant instanceof SunShroom && chooseMap.getMapNumber() == 1){
                                ((SunShroom)wantedPlant).setSleep(true);
                            }
                            if (wantedPlant instanceof SunFlower && chooseMap.getMapNumber() == 2){
                    ...
                    if (plantNumber < 8) {
                        switch (plantList.get(plantNumber)) {
                            ...
                            case "sunShroom": {
                                ...
                                } else sunShroomGif = new Image(new File("@../../images/Plants/SunShroom/SunShroom2.gif").toURI().toString());
                        ...
                        if (!isPause) {
                            for (entity.zombie.Zombie zombie : zombies) {
                                if (zombie.getHealth() > 0) {
                                    ...
                                    if (allPlants.size() != 0) {
                                        ...
                                        for (Plant plant : allPlants) {
                                            if (plant.getImageView().getImage() != null) {
                                                if (plant.getHealth() > 0) {
                                                    zombie.attack(plant, normalCallBack);
                                                }
                                        ...
                                        for (Plant plant : allPlants) {
                                            if (plant.getHealth() <= 0) {
                                                if (plant instanceof TallNut || plant instanceof EatFlower) {
                                                    plant.getImageView().setY(plant.getImageView().getY() + 20);
                                                }
                                    ...
                                    //僵尸移动
                                    if (zombie.isToWalk()) {
                                        zombie.move(zombie.getVelocity());//2
                                    }

                                    //判断是否有子弹打到
                                    for (Bullet bullet : allBullets) {
                                        bullet.attack(zombie, normalCallBack);
                                    }
                            ...
                            for (ImageView imageView : grayImageviews) {
                                ...
                                if (imageView.getLayoutY() <= -68.5) {
                                    ...
                                    normalMapPane.getChildren().remove(imageView);
                                }
                            }
                        }
                    } else {
    ...
    private void playAgain() throws IOException {
        ...
        normalMapPane.getChildren().setAll(newPane);
    }

    @FXML
    private void backMenu() throws IOException {
        javafx.scene.layout.AnchorPane menuPane = FXMLLoader.load(getClass().getResource("menu.fxml"));
        normalMapPane.getChildren().setAll(menuPane);
        audioPlayer.interrupt();
        timer.cancel();
    }

    //回调内部类
    public class NormalCallBack extends PrimaryCallBack {
        public void addSun(Sun sun) {
            sunes.add(sun);
            normalMapPane.getChildren().add(sun.getSunImage());
        }
        ...
        public void addZombie(Zombie zombie) {
            ...
            zombies.add(zombie);
            normalMapPane.getChildren().add(zombie.getImageView());
            zombieNumber = zombieNumber + 1;
        }

        public void addPeaBullet(PeaBullet peaBullet) {
            allBullets.add(peaBullet);
            normalMapPane.getChildren().add(peaBullet.getImageView());
        }
        ...
        public void decreaseZombie(int row) {
            ...
            for (int anIsHaveZombie : isHaveZombie) {
                theRestZombie = anIsHaveZombie + theRestZombie;
            }
        }
    }
    ...
    private void shovelMouseMoved() {
        ...
        isShovel = true;
        mouseShovel.setImage(shovelImage);
        normalMapPane.setOnMouseMoved(event -> {
    ...
    private void beGray(ImageView imageView)//1
    {
        ...
        normalMapPane.getChildren().add(grayImageView);
    }
